import pytest
from interviews.interview_02A import find_intersection
...
def test_find_intersection():
    """find_intersection(head1: Node, head2: Node) -> Node:"""
    # Create two linked lists that intersect at node 5
    head1 = Node(1, Node(2, Node(3, Node(4, Node(5)))))
    head2 = Node(6, Node(7, Node(8, Node(9, head1.next.next))))

    # Find the intersection
    intersection = find_intersection(head1, head2)

    # Verify the intersection
    assert intersection.value == 3
    assert intersection.next.value == 4
    assert intersection.next.next.value == 5
